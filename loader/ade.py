...
from loader.segbase import SegmentationDataset
...
class ADE20KSegmentation(SegmentationDataset):
    ...
    def __init__(self, root='/home/xiangjianjian/Projects/spectral-setr/dataset/', split='test', mode=None, transform=None, **kwargs):
        super(ADE20KSegmentation, self).__init__(
            root, split, mode, transform, **kwargs)
        root = os.path.join(root, self.BASE_DIR)
        assert os.path.exists(
            root), "Please setup the dataset using ../datasets/ade20k.py"
        self.images, self.masks = _get_ade20k_pairs(root, mode=mode)
