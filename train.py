...
class Trainer(object):
    ...
    def train(self):
        ...
        lr_scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, step_size=10, gamma=0.5, last_epoch=self.last_step//self.eval_steps)
        self.model.to(self.device)
        train_iter = iter(self.train_loader)
        report_loss = 0
        for step in tqdm(range(self.last_step, self.total_steps), desc=f"Train", ncols=150):
            ...
            if (step+1) % self.eval_steps == 0:
                ...
                self.logger.plot_acc(
                    self.step_list, self.mIoU_list, self.mf1_list)
    ...
    def eval(self):
        ...
        with torch.no_grad():
            for val_img, val_mask in tqdm(
                self.test_loader, desc="Valid", ncols=150
            ):
    ...
    def _unpack_train_config(self, train_config):
        ...
        self.last_step = self.last_step if self.last_step else 0
    ...
    def _restore(self):
        self.model.load_state_dict(
            torch.load(self.restore_path, map_location="cpu")
        )
        print(f"Restored from {self.last_step} step!")
        self.logger.info(f"Restored from {self.last_step} step!")
