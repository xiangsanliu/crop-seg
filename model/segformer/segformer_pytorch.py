...
class EfficientSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = map(lambda t: rearrange(
            t, 'b (h c) x y -> (b h) (x y) c', h=heads), (q, k, v))
        ...
        for (dim_in, dim_out), (kernel, stride, padding), num_layers, ff_expansion, heads, reduction_ratio in zip(dim_pairs, stage_kernel_stride_pad, num_layers, ff_expansion, heads, reduction_ratio):
            ...
            for _ in range(num_layers):
                layers.append(nn.ModuleList([
                    PreNorm(dim_out, EfficientSelfAttention(
                        dim=dim_out, heads=heads, reduction_ratio=reduction_ratio)),
                    PreNorm(dim_out, MixFeedForward(
...
class Segformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        dims, heads, ff_expansion, reduction_ratio, num_layers = map(partial(
            cast_tuple, depth=4), (dims, heads, ff_expansion, reduction_ratio, num_layers))
    ...
    def forward(self, x):
        ...
        fused = [to_fused(output) for output, to_fused in zip(
            layer_outputs, self.to_fused)]
