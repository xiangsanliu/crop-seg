...
if __name__ == "__main__":
    ...
    new_w = (
        (w // target_w) * target_w
        if (w // target_w == 0)
        else (w // target_w + 1) * target_w
    )
    new_h = (
        (h // target_h) * target_h
        if (h // target_h == 0)
        else (h // target_h + 1) * target_h
    )
    # new_w = new_w - target_w
    # new_h = new_h - target_h
    ...
    if label_path is not None:
        label = cv.copyMakeBorder(
            label, 0, new_h - h, 0, new_w - w, cv.BORDER_CONSTANT, 0
        )
    ...
    #     image, stride // 2, stride // 2, stride // 2, stride // 2, cv.BORDER_CONSTANT, 0
    # )
    # if label_path is not None:
    #     label = cv.copyMakeBorder(
    #         label,
    ...
    def crop(cnt, crop_image, crop_label):
        ...
        if crop_label is not None:
            label_name = os.path.join(
                save_label_dir, basename + "_" + str(cnt) + ".png"
            )
